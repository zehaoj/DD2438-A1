...
namespace Scrips
{
    public class Node<T> : IEqualityComparer, IEnumerable<T>, IEnumerable<Node<T>>
    {
        ...
        public void Add(Node<T> childNode, int index = -1)
        {
            ...
            if (index > Children.Count() - 1)
            {
                throw new ArgumentException(
                    "The index ({0}) can not be higher then index of the last iten. Use the AddChild() method without an index to add at the end".FormatInvariant(
                        index
                    )
                );
            }
        ...
        public void AddParent(Node<T> parentNode)
        {
            ...
            {
                throw new ArgumentException(
                    "This node [{0}] already has a parent".FormatInvariant(Value),
                    "parentNode"
                );
            }
            parentNode.Add(this);
        ...
        public void Disconnect()
        {
            if (IsRoot)
            {
                throw new InvalidOperationException(
                    "The root node [{0}] can not get disconnected from a parent.".FormatInvariant(
                        Value
                    )
                );
            }
        ...
        public static IEnumerable<Node<T>> CreateTree<TId>(
        ...
        ) where TId : struct
        {
            ...
            if (!valuesCache.Any())
                return Enumerable.Empty<Node<T>>();
            T itemWithIdAndParentIdIsTheSame = valuesCache.FirstOrDefault(
                v => IsSameId(idSelector(v), parentIdSelector(v))
            );
        ...
        ) where TId : struct
        {
            ...
            foreach (var rootNode in rootNodesCache)
            {
                ...
                var parent = rootNodesCache.FirstOrDefault(
                    n => IsSameId(idSelector(n.Value), parentId)
                );
                ...
                {
                    throw new ArgumentException(
                        "A value has the parent ID [{0}] but no other nodes has this ID".FormatInvariant(
                            parentId.Value
                        )
                    );
                }
