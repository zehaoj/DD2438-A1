    ...
    public class Node<T> : IEqualityComparer, IEnumerable<T>, IEnumerable<Node<T>>
        {
          ...
          public void Add(Node<T> childNode, int index = -1)
          {
              ...
              if (!childNode.IsRoot)
              {
                  throw new ArgumentException($"The child node with value [{childNode.Value}] can not be added because it is not a root node.");
              }
   
              if (Root == childNode)
              {
                  throw new ArgumentException($"The child node with value [{childNode.Value}] is the rootnode of the parent.");
              }
   
              if (childNode.SelfAndDescendants.Any(n => this == n))
              {
                  throw new ArgumentException($"The childnode with value [{childNode.Value}] can not be added to itself or its descendants.");
              }
          ...
          public void AddParent(Node<T> parentNode)
          {
              if (!IsRoot)
              {
                  throw new ArgumentException($"This node [{Value}] already has a parent", "parentNode");
              }
              parentNode.Add(this);
          }
